...
class Sampler:
    ...
    def initialize(self, conf: DictConfig) -> None:
        ...
        self.diffuser = Diffuser(
            **self._conf.diffuser, cache_dir=f"{SCRIPT_DIR}/../schedules"
        )
    ...
    def load_model(self):
        ...
        model = RoseTTAFoldModule(
            **self._conf.model,
            ...
            cyclic_peptide=self._conf.inference.cyclic_peptide,
        ).to(self.device)
    ...
    def sample_init(self, return_forward_trajectory=False):
        ...
        if self.diffuser_conf.partial_T:
            ...
            xyz_mapped = get_init_xyz(
                xyz_mapped, center=self.symmetry is None
            ).squeeze()
